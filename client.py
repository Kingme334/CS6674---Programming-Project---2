...
def generate_k_and_kstar(a: int, b: int):
    """Generate random k in [1..1,000,000] until k* != 0."""
    while True:
...
def _permute(bits: int, table: list, in_size: int) -> int:
    out = 0
    for pos in table:
        out = (out << 1) | ((bits >> (in_size - pos)) & 1)
    return out

def _left_rotate28(x: int, n: int) -> int:
    x &= (1 << 28) - 1
    return ((x << n) | (x >> (28 - n))) & ((1 << 28) - 1)

def _des_subkeys(key64: int) -> list[int]:
    key56 = _permute(key64, PC1, 64)
    C = (key56 >> 28) & ((1 << 28) - 1)
    D = key56 & ((1 << 28) - 1)
    subkeys = []
    for s in SHIFTS:
        C = _left_rotate28(C, s)
        D = _left_rotate28(D, s)
        cd = (C << 28) | D
        subkeys.append(_permute(cd, PC2, 56))  # 48-bit
    return subkeys

def _f(R32: int, K48: int) -> int:
    ER = _permute(R32, E, 32)  # 48
    x = ER ^ K48
    out32 = 0
    for i in range(8):
        chunk = (x >> (42 - 6*i)) & 0x3F
...
def des_encrypt_block(block64: int, key64: int) -> int:
    subkeys = _des_subkeys(key64)
    x = _permute(block64, IP, 64)
    L = (x >> 32) & 0xFFFFFFFF
    R = x & 0xFFFFFFFF
    for i in range(16):
        L, R = R, (L ^ _f(R, subkeys[i])) & 0xFFFFFFFF
    preout = (R << 32) | L
    return _permute(preout, FP, 64)

def _clean_hex_line(line: str) -> str:
    return "".join(line.strip().split()).upper()

def des_encrypt_hex_line(hexline: str, key_hex: str) -> str:
    hexline = _clean_hex_line(hexline)
    if not hexline:
        return ""
    if len(hexline) % 16 != 0:
        raise ValueError(f"DES input line must be multiple of 16 hex chars. Got {len(hexline)}.")
    key64 = int(key_hex, 16)
    out = []
    for i in range(0, len(hexline), 16):
        block = int(hexline[i:i+16], 16)
        c = des_encrypt_block(block, key64)
        out.append(f"{c:016X}")
    return "".join(out)
...
def main():
    if len(sys.argv) < 2:
        ...
        print("Example: python3 client.py 127.0.0.1")
        sys.exit(1)
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        if choice == "1":
            ...
            while True:
                plain = input("Enter plaintext (or 'quit'): ")
                if plain.strip().lower() == "quit":
                    s.sendall(b"quit\n")
                    ack = f.readline().strip()
                    print("[CLIENT] Server:", ack)
                    break

                cipher = shift_encrypt(plain, k_star)
                print(f"(plaintext, ciphertext) = ({plain}, {cipher})")
                s.sendall((cipher + "\n").encode("utf-8"))
                ack = f.readline().strip()
                print("[CLIENT] Server:", ack)
            ...
            print(f"[CLIENT] DES mode using key {KEY_HEX}")
            try:
                with open(filename, "r", encoding="utf-8") as infile:
                    for raw in infile:
                        line = _clean_hex_line(raw)
                        if not line:
                            continue
                ...
                print(f"[CLIENT] ERROR: File not found: {filename}")
